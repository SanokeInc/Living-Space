...
public class EndScreen implements Screen {
	...
	private boolean isWithinMenu(float x, float y) {
		return ((x >= MENU_BUTTON_P1_X && x <= MENU_BUTTON_P2_X)
				&& (y >= MENU_BUTTON_P1_Y && y <= MENU_BUTTON_P2_Y) && (timer > 1.99f)) ? true
				: false;
	}
	...
	private boolean isWithinQuit(float x, float y) {
		return ((x >= QUIT_BUTTON_P1_X && x <= QUIT_BUTTON_P2_X)
				&& (y >= QUIT_BUTTON_P1_Y && y <= QUIT_BUTTON_P2_Y) && (timer > 1.99f)) ? true
				: false;
	}
	...
	public Texture getImage(float delta) {
		...
		if (timer < 1.99f) {
			...
			image = Assets.endScreenFrames[currentFrameNumber
					/ FRAME_SWITCH_DELAY];
			return imageToShow;
		} else {
			Texture imageToShow = image;
			currentFrameNumber = (currentFrameNumber + 1)
					% (4 * FRAME_SWITCH_DELAY);
			image = Assets.endScreenFrames[currentFrameNumber
					/ FRAME_SWITCH_DELAY];
			return imageToShow;
		}
